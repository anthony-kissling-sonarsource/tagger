...
def printHelp():
    print 'Syntax : removetags tag files, [files], [files], ...'
    print 'tag : Tag à appliquer'
    print 'files : fichiers sur lesquels on supprime le tag'
...
if len(sys.argv) > 2:
    ...
    for i in range(2, len(sys.argv)):
        if os.path.exists(os.path.realpath(path + delimiter + sys.argv[i])):
            files.append(os.path.realpath(path + delimiter + sys.argv[i]))
        else:
            files.append(os.path.realpath(sys.argv[i]))
else:
    printHelp();
